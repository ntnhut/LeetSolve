...
class Solution {
public:
    void insertBuy(list<pair<int, size_t>>& buyBacklog, int price, size_t amount) {
        auto it = buyBacklog.begin();
        while (it != buyBacklog.end() && it->first > price) {
            ++it;
        }

        if (it != buyBacklog.end() && it->first == price) {
            it->second += amount;
            return;
        }

        buyBacklog.insert(it, {price, amount});
    }

    void insertSell(list<pair<int, size_t>>& sellBacklog, int price, size_t amount) {
        auto it = sellBacklog.begin();
        while (it != sellBacklog.end() && it->first < price) {
            ++it;
        }

        if (it != sellBacklog.end() && it->first == price) {
            it->second += amount;
            return;
        }

        sellBacklog.insert(it, {price, amount});
    }

    int getNumberOfBacklogOrders(vector<vector<int>>& orders) {
        // [price, amount], who bought with higher prices will be executed first
        list<pair<int, size_t>> buyBacklog;
        // who sold with lower prices will be executed first
        list<pair<int, size_t>> sellBacklog;
        ...
        for (auto& order : orders) { // can't use [price, amount, type]
            ...
            if (type == 0) { // buy
                ...
                if (amount > 0) { // add remaining to backlog
                    insertBuy(buyBacklog, price, amount);
                    totalAmount += amount;
                }
                ...
                if (amount > 0) { // add remaining to backlog
                    insertSell(sellBacklog, price, amount);
                    totalAmount += amount;
                }
            }
        }
